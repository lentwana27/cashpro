...
const modalRegex = /function ExchangeRatesModal\(\{ rates, onClose, onUpdate \}: any\) \{[\s\S]*?(?=const BreakdownSection)/;
let match = content.match(modalRegex);
if (!match) {
    console.error("Modal not found");
    process.exit(1);
}

let newModalCode = `function ExchangeRatesModal({ rates, onClose, onUpdate }: any) {
...
console.log("Updated ExchangeRatesModal");
