...
if (!code.includes('handleClearPastData')) {
  code = code.replace(/const requestUnlock = async \(dStr: string\) => \{/g, `
  ...
  code = code.replace(/<p className="text-slate-400 text-sm mt-1">\s*Branch Code: \{user\?\.branchId\}\s*<\/p>\s*<\/div>\s*<div>/g, `<p className="text-slate-400 text-sm mt-1">
